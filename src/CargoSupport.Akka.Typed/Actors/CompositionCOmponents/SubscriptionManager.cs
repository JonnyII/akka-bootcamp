...
internal class SubscriptionManager : IDisposable
{
    ...
    ///     multiple message of the same type are handled by comparing the <see cref="sender" /> with the result of
    ///     <see cref="getSender" />.
    ///     this should enable us to handle the same message from different senders without handling the events for the wrong
    ///     actors
    /// </summary>
    ...
    {
        return Observable.Create((IObserver<TEvent> observer) =>
        {
            addReceiveHandler(
